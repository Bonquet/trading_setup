...
input double  Trend_Separation_ATR  = 0.5;
input int     ATR_Period            = 14;
input double  Thrust_Range_Mult     = 1.2;
input double  Thrust_Body_Ratio     = 0.7;
input double  Pullback_Body_Ratio   = 0.6;
input int     Active_Hour_Start     = 8;
...
//==================== Signal ====================================//
void CheckSignal() {
   // Need EMA_Slope_Lookback+2 EMA50 history bars
   double ema_fast_hist[];
   double ema_slow_hist[];
   double atr_hist[];
   ArraySetAsSeries(ema_fast_hist, true);
   ArraySetAsSeries(ema_slow_hist, true);
   ArraySetAsSeries(atr_hist, true);

   int slope_n = EMA_Slope_Lookback + 5;
   if (CopyBuffer(h_ema_fast, 0, 1, slope_n, ema_fast_hist) <= 0) return;
   if (CopyBuffer(h_ema_slow, 0, 1, 3, ema_slow_hist) <= 0) return;
   if (CopyBuffer(h_atr,      0, 1, 3, atr_hist)      <= 0) return;

   double ef_now  = ema_fast_hist[0];
   double ef_back = ema_fast_hist[EMA_Slope_Lookback];
   double es_now  = ema_slow_hist[0];
   double atr_now = atr_hist[0];
   ...
   bool bull = (cl > op);
   double body_ratio = body / bar_range;

   bool thrust = (bar_range > Thrust_Range_Mult * atr_now)
              && bull
              && (body_ratio > Thrust_Body_Ratio);

   bool pullback = (lo <= ef_now)
                && (cl > ef_now)
                && bull
                && (body_ratio > Pullback_Body_Ratio);

   if (!thrust && !pullback) return;

   string trigger_name;
   if (thrust) trigger_name = "THRUST";
   else        trigger_name = "PULLBACK";
   ...
   string comment = Trade_Comment + ":" + trigger_name;
   if (g_trade.Buy(lots, g_symbol, 0, sl, tp, comment)) {
...
void ResetDailyCounterIfNewDay() {
   ...
   string today_str = StringFormat("%04d.%02d.%02d 00:00", dt.year, dt.mon, dt.day);
   datetime today_at_midnight = StringToTime(today_str);
